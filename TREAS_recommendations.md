# TREAS Cluster Operational Review
...
This cluster looks dramatically different from SSA. Let me walk through it.
...
Every single index in TREAS has either zero documents or, in the case of `cdm_cpe_dictionary` (1.49M docs) and a few Kibana/system indices, store sizes that round to 0 GB. The largest user-data shard footprint anywhere is 250 MB (split between `cdm_cpe_dictionary` and one SLM history datastream backer on `instance-0000000008` and `instance-0000000011`).
...
1. **Enrich indices replicated to every hot node.** You have 4 enrich policies (`cdm_config_t0`, `cdm_software_directive`, `cdm_csm_stig`, `cdm_software_dictionary_eol`, `cdm_vuln_cve` — actually 5) × 12 nodes = ~60 shards just for enrich. That's roughly a quarter of all shards in the cluster, all for ingest pipeline lookups.
...
This is where TREAS really stands out: **57 of the ~58 user-visible indices are empty**. The only index with meaningful document count is `cdm_cpe_dictionary` (1.49M docs from CPE seed data). Even `data_dictionary` is small reference data, and the rest is at 0 docs.

Categorized:

**Kibana Alerting framework (14 indices)** — `.internal.alerts-*-default-000001` covering security, ML anomaly detection (×2), observability (metrics/logs/uptime/threshold/slo/apm), streams, stack, transform health, dataset quality, attack discovery, default. Same as SSA — auto-created by Kibana, expected to be empty until rules fire. Leave them alone.
...
**CDM application indices (24 indices)** — `cdm_vuln`, `cdm_vuln_remediated`, `cdm_vuln_trending-6-000001`, `cdm_cyhy_*`, `cdm_scuba_m365_*`, `cdm_scuba_gws_*`, `cdm_asm_nmi`, `cdm_vdp`, `cdm_threat`, `cdm_hwam`, `cdm_software_dictionary`, `cdm_kev_dictionary`, `cdm_stig_dictionary`, `cdm_cve_dictionary`, `cdm_directive_dictionary`, `cdm_benchmark_vuln`, `cdm_system_boundary_fisma_1163_rollup`, `cdm_cyhy_host_scans`, `cdm_cyhy_domain*`, `cdm_cyhy_vuln`. **All zero docs.** Even the dictionaries that were populated in SSA (`cdm_cve_dictionary` had 347K, `cdm_stig_dictionary` had 84K, `cdm_software_dictionary` had 1.68M, `cdm_kev_dictionary` had 1.5K) are all empty here.

**Other (1 each):** `.kibana_locks-000001`, `.elastic-connectors*` not present (good — TREAS doesn't have the Enterprise Search overhead SSA has).
...
**No ILM activity.** The `.ds-ilm-history-7-*` datastreams are tiny (single digits of docs in the most recent backing index, vs. SSA's `2026.04.29-000011` reaching #11 with hundreds of docs). ILM has done almost nothing on this cluster because there's nothing to manage.
...
**Frozen disk percent reads 95.57% on both nodes despite 0 indices.** Same pattern as the SSA frozen node. This is an ESS reporting artifact (overcommit math on `disk.total` vs `disk.avail`), not a real signal here. It would be a real signal if the frozen tier actually held shards — but on TREAS, both frozen nodes are completely empty.

**Significantly less cluster-state bloat than SSA.** TREAS doesn't have the `.ent-search-*` family at all (no Enterprise Search / App Search / Workplace Search artifacts visible in the shard listing), which is good — it means this deployment skipped that feature. Given that Enterprise Search is removed in 9.0, TREAS is in better shape for that upgrade than SSA.
...
**`cdm_vuln_trending-6-000001` naming inconsistency.** Note the index is `cdm_vuln_trending-6-000001` (six digits) here, while in SSA it's `cdm_vuln_trending-6-0000001` (seven digits). Not a problem in itself — different rollover number-padding configurations — but if you have shared tooling that targets these by exact name pattern, the `?` glob length may matter. Worth a quick check.

---

## Comparison vs SSA in one paragraph

SSA is a mature, actively-ingesting production cluster with 322 primaries, ~600 GB of real data, working trending pipelines, and a mix of legitimate operational issues (a few over-sharded indices, some broken pipelines for SCuBA GWS/VDP/ASM, Enterprise Search bloat). **TREAS is the opposite — a structurally healthy but fundamentally idle cluster** that was provisioned in a single window on 2026-03-25 and never received its data feed.

## Suggested actions

1. **Verify expected state.** Is TREAS supposed to be ingesting? If yes, this is the headline finding — every CDM ingestion path is silent. Six weeks of empty trending datastreams (still on backing index `-000001`) suggests no documents have ever arrived.
2. **If TREAS is intentionally pre-prod / standby:** the current state is fine. No remediation needed.
3. **If TREAS should be receiving data:** check the upstream Logstash / pipeline configuration pointing at this cluster. Given your CDM pipeline silent-failure history (zero-document ingestion with `Connection reset by peer` errors), this looks like the same failure mode at higher scale — every CDM target. I'd start with the Logstash output config and any CCS / cross-cluster replication settings if data is supposed to be flowing in from another cluster.
...
Happy to dig deeper into any of these — particularly the ingestion-path diagnosis if TREAS is supposed to be live, or to compare specific index templates between SSA and TREAS to confirm whether the empty CDM indices are using the same mappings you'd expect to flow data into.
