...
This deployment looks essentially idle from a data perspective. Let me walk through it.
...
**No resharding is warranted on the data side**, because there's no data to reshard. Every CDM index is at `pri=1, rep=1`, which is the right default for 0 GB content. If/when data starts flowing in, you'll want to revisit primary counts based on actual ingest volume against the same 40 GB/shard target.
...
- 242 total shards across 6 hot nodes = **~40 shards per hot node**. Against a ceiling of ~592 shards/node (20 shards per GB of 29.6 GB heap), you're at ~6.8% of the safe limit.
...
1. **Enrich indices replicated to every hot node.** You have 4 enrich policies (`cdm_config_t0`, `cdm_software_directive`, `cdm_csm_stig`, `cdm_software_dictionary_eol`, `cdm_vuln_cve`) × 6 nodes = ~30 shards just for enrich. That's roughly 12% of all shards in the cluster, all for ingest pipeline lookups.
...
**Frozen tier:** All three frozen nodes (`instance-0000000012`, `instance-0000000013`, `instance-0000000014`) show **0 shards, 0 bytes of disk indices**. They report `disk.used_percent: 90.08%` — that's an ESS reporting artifact (overcommit math on `disk.total` vs `disk.avail`), not a real signal here. It would be a real signal if the frozen tier actually held shards — but in this deployment, all three frozen nodes are completely empty. With no data ever aged into frozen, three frozen nodes is significant unused capacity.
...
**No ILM activity beyond housekeeping.** The `.ds-ilm-history-7-*` datastreams are tiny. The most recent backing index is `.ds-ilm-history-7-2026.04.22-000003` with 0 docs. ILM has done almost nothing on this cluster because there's nothing to manage. Trending datastreams are still all on backing index `-000001` six weeks after creation — no rollovers have occurred because no documents have arrived to age out.
...
**Frozen tier is unused but expensive.** Three frozen nodes (each 600 GB at 90% reported usage but 0 actual shards) is significant overprovisioning if nothing is ever going to age there. If this cluster is intentionally pre-prod and will eventually have data flowing through ILM into frozen, leave them alone. If not, you could potentially scale the frozen tier down and save cost.

**Index naming.** `cdm_vuln_trending-6-0000001` uses seven-digit zero-padding for the rollover number (matching the SSA convention rather than the six-digit pattern seen in some other deployments). If you have shared tooling across deployments that targets these by exact name pattern, the glob length may matter. Worth a quick check that index templates and downstream consumers match.
...
6. **Right-size the frozen tier.** Three frozen nodes for zero frozen shards is overprovisioned. Either confirm a near-term plan to use them, or scale down to one (matching the typical pattern in your other deployments).
